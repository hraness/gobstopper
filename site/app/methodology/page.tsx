      ...
      <main id="main" tabIndex={-1} className="document-page">
        <article>
          ...
          <ul>
            ...
            <li><strong>cliff</strong> keeps the head and the newest three assistant steps byte-for-byte and drops every older tool result over 500 bytes. It has no floor and adds no state card; the <a href="/compare/cliffcompaction">comparison with CliffCompaction</a> explains where the rule comes from. For running Claude Code and Codex sessions, <code>gobstopper proxy</code> applies the rule to each outgoing request instead of a copy.</li>
