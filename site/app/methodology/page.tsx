      ...
      <main id="main" tabIndex={-1} className="document-page">
        <article>
          ...
          <ul>
            ...
            <li><strong>cliff</strong> keeps the head, the newest three assistant steps, and the newest eight tool results byte-for-byte and drops older eligible tool results over 500 bytes. It has no floor and adds no state card; the <a href="/compare/cliffcompaction">comparison with CliffCompaction</a> explains where the rule comes from. For running sessions in Claude Code, Codex, and OpenAI-compatible agents, <code>gobstopper proxy</code> applies the rule to each outgoing request instead of a copy.</li>
