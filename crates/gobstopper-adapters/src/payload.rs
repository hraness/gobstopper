...
pub fn text_bytes(value: &Value) -> u64 {
    match value {
        ...
        Value::Array(blocks) => blocks.iter().filter(|b| text_block(b))
...
pub fn elide(value: &mut Value, stub: String) -> u64 {
    ...
    if candidate.to_string().len() >= value.to_string().len() { return 0; }
    *value = candidate;
    old
}
